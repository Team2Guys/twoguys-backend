...
export class SalesProductsService {
  ...
  async get_all_records() {
    try {
      ...
      let totalorders = await this.prisma.salesProducts.count({
        where: { paymentStatus: true },
      });
      let Total_abandant_order = await this.prisma.salesProducts.count({
  ...
  async postpaymentStatus(postpayment: PaymentQueryDto) {
    try {
      ...
      const products: ProductInput[] = JSON.parse(
        JSON.stringify(existingOrder.products),
      ) as ProductInput[];
  ...
  async getLast7DaysStats() {
    ...
    const dailyOrdersMap = orders.reduce(
      (acc, order) => {
